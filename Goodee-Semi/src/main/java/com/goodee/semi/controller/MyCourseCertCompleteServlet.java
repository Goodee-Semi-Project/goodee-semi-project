...
@WebServlet("/myCourse/certComplete")
public class MyCourseCertCompleteServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
	private CourseService courseService = new CourseService();
	private PetService petService = new PetService();
	private ClassService classService = new ClassService();
       
  public MyCourseCertCompleteServlet() {
    super();
  }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (Course course : courseList) {
			...
			for (Pet pet : myPetInCourse) {
				...
				courseByPet.setClassData(classService.selectClassByCourseNoAndPetNo(courseByPet.getCourseNo(), pet.getPetNo()));
				
				if (courseByPet.getClassData().getClassProg() == 100) {
		...
		request.getRequestDispatcher("/WEB-INF/views/myCourse/certComplete.jsp").forward(request, response);
	}
